...
context('mixinsplus', function()
	...
	context('Chained methods', function()
		context('absent from the mixin declaring them', function()
			test("should not raise an error or affect class behavior.", function()
				instance = aclass:extend():with(chain1)()
				instance:bar()
		...
		context('run in the order their mixins are declared', function()
			context('with varargs', function()
				before(function()
					instance = aclass:extend():with(chain1, chain2)()
				end)
				ChainTests()
			end)
			context('with piped methods', function()
				before(function()
					instance = aclass:extend():with(chain1):with(chain2)()
				end)
				ChainTests()
			end)
		end)
	...
	context('Intercepting methods', function()
		context('absent from the mixin declaring them', function()
			test("should not raise an error or affect class behavior.", function()
				instance = aclass:extend():with(intA)()
				instance:bar()
				assert_equal(instance.barV, "XX")
			end)
		end)
		context('with varargs', function()
			before(function()
				c1 = aclass:extend():with(intA)
				c2 = aclass:extend():with(intA, intB)
			end)
			intTests()
		end)
		context('with piped methods', function()
			before(function()
				c1 = aclass:extend():with(intA)
				c2 = aclass:extend():with(intA):with(intB)
			end)
			intTests()
		end)
	end)
	...
	context('Using both kind of special methods', function()
		...
		context('with different mixins', function()
			context('with varargs', function()
				before(function()
					instance =
						aclass:extend():with(chain1, chain2, intC, intD)()
				end)
				mixTests()
			end)
			context('with piped methods', function()
				before(function()
					instance = aclass:extend()
						:with(chain1):with(chain2):with(intC):with(intD)()
				end)
				mixTests()
			end)
		end)
	...
	context('Working with sub classes', function()
		test('should be commutative if both types get the same order', function()
			local bclass = aclass:extend():with(chain1)
			local cclass = bclass:extend():with(chain2)
			local instD  = cclass:extend():with(intD  )()
			local dclass = aclass:extend():with(chain1)
			local eclass = dclass:extend():with(intD  )
			local instF  = eclass:extend():with(chain2)()
			local fclass = aclass:extend():with(intD  )
			local gclass = fclass:extend():with(chain1)
			local instH  = gclass:extend():with(chain2)()
			instD:ChainA()
			instF:ChainA()
			instH:ChainA()
		...
		test('and give the same results as with a single class.', function()
			local bclass = aclass:extend():with(chain1)
			local cclass = bclass:extend():with(chain2)
			local dclass = cclass:extend():with(intC)
			local instE  = dclass:extend():with(intD)()
			local instX = aclass:extend()
				:with(chain1):with(chain2):with(intC):with(intD)()
			instE:ChainA()
			instX:ChainA()
